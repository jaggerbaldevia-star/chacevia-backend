// api/pdf.js
//
// Merged PDF-builder endpoints (moved here to stay under Vercel's
// serverless function cap): notes-pdf, direction-pdf, study-set.
// Dispatches on an action discriminator, read as
// (req.query.action || req.body?.action). Each action's handler below
// is the original standalone file's logic, unchanged.
...
function wrapNotesPdf(text, font, size, maxW) {
...
async function handleNotesPdf(req, res) {
    ...
    try {
        ...
        const line = (text, font, size, color, gap, indent) => {
            const lines = wrapNotesPdf(text, font, size, maxW - (indent || 0))
...
function wrapDirectionPdf(text, font, size, maxW) {
    const words = String(text || "").split(/\s+/).filter(Boolean)
    const lines = []
    let line = ""
    for (const w of words) {
...
async function buildBriefPdf(idea, text) {
    const pdf = await PDFDocument.create()
    const reg = await pdf.embedFont(StandardFonts.Helvetica)
    const bold = await pdf.embedFont(StandardFonts.HelveticaBold)
    const ital = await pdf.embedFont(StandardFonts.HelveticaOblique)
    ...
    const draw = (t, font, size, color, gap, indent) => {
        for (const ln of wrapDirectionPdf(t, font, size, maxW - (indent || 0))) {
            ensure(size + 4)
            page.drawText(ln, { x: M + (indent || 0), y: y - size, size, font, color })
            y -= size + (gap || 4)
        }
...
const STUDY_COIN_COST = 4
const STUDY_DEFAULT_MODEL = MODELS.smart
const STUDY_TRANSCRIBE_MODEL = "whisper-1"

const STUDY_INSTRUCTIONS = `You are Chacevia's study coach. You get the transcript of a lecture, class, or study recording. Build a complete study kit from it.
...
function extractParsedStudySet(text) {
    let t = String(text || "").trim().replace(/^```json/i, "").replace(/^```/, "").replace(/```$/, "")
    const a = t.indexOf("{"), b = t.lastIndexOf("}")
    return JSON.parse(t.slice(a, b + 1))
}

function wrapStudySet(text, font, size, maxW) {
    const words = String(text || "").split(/\s+/).filter(Boolean)
    const lines = []
    let line = ""
    for (const w of words) {
...
async function buildStudyKitPdf(data) {
    const pdf = await PDFDocument.create()
    const reg = await pdf.embedFont(StandardFonts.Helvetica)
    const bold = await pdf.embedFont(StandardFonts.HelveticaBold)
    const ital = await pdf.embedFont(StandardFonts.HelveticaOblique)
    ...
    const draw = (t, font, size, color, gap, indent) => {
        for (const ln of wrapStudySet(t, font, size, maxW - (indent || 0))) {
            ensure(size + 4)
            page.drawText(ln, { x: M + (indent || 0), y: y - size, size, font, color })
            y -= size + (gap || 4)
        }
...
async function handleStudySet(req, res) {
    ...
    try {
        const guard = await requireCoins(req, body, STUDY_COIN_COST, "study-set")
        if (!guard.ok) return res.status(guard.status).json(guard.payload)

        const clean = audioB64.replace(/^data:[^;]+;base64,/, "")
        const buffer = Buffer.from(clean, "base64")
        const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY })
        const model = process.env.OPENAI_MODEL || STUDY_DEFAULT_MODEL

        // 1) Transcribe
        let transcript = ""
        try {
            const file = await toFile(buffer, filename)
            const tr = await withRetry(() => openai.audio.transcriptions.create({ file, model: STUDY_TRANSCRIBE_MODEL }), { label: "study-transcribe" })
        ...
        let data
        try { data = extractParsedStudySet(resp.output_text) } catch (e) {
...
export default async function handler(req, res) {
    const action = (req.query && req.query.action) || (req.body && typeof req.body === "object" && req.body.action)

    if (action === "direction-pdf") return handleDirectionPdf(req, res)
